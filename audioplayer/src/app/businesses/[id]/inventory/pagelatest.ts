...
export default function BusinessInventory({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  // --- NEW SUPPLIER STATE FIELDS ---
  const [suppliers, setSuppliers] = useState<any[]>([]);
  const [supplierSearch, setSupplierSearch] = useState('');
  const [newSupplierName, setNewSupplierName] = useState('');
  const [newSupplierStatus, setNewSupplierStatus] = useState<'active'|'inactive'>('active');
  const [newSupplierContact, setNewSupplierContact] = useState(''); // Added contact name
  const [newSupplierEmail, setNewSupplierEmail] = useState('');     // Added email
  const [newSupplierPhone, setNewSupplierPhone] = useState('');     // Added phone
  const [newSupplierLocation, setNewSupplierLocation] = useState(''); // Added location
  const [newSupplierCategory, setNewSupplierCategory] = useState(''); // Added category/type
  // ---------------------------------
  ...
  const handleAdd = async (e: React.FormEvent) => {
    ...
    try {
      // UnitPrice is now always available in state, passed to API call
  ...
  const handleCreateSupplier = async (e: React.FormEvent) => {
    ...
    try {
      // Updated createSupplier call with new fields
      const sup = await createSupplier(user, id, { 
        name: newSupplierName, 
        status: newSupplierStatus,
        contact: newSupplierContact,
        email: newSupplierEmail,
        phone: newSupplierPhone,
        address: newSupplierLocation, // Assuming 'address' field in business API can be used for 'Location'
        category: newSupplierCategory // Assuming 'category' or 'type' can be passed
      });
  ...
  // (other functions like loadReport, handleCreateCategory, handleCreateUnit, handleStockIn remain the same)
  ...
  return (
    <div className="p-6 text-white min-h-screen bg-black">
      ...
      <div className="mt-6">
        ...
        {/* Items Tab - Add Form (Revised layout with Unit Price) */}
        {tab === 'items' && (
          <GlassCard className="mb-6">
            ...
            <form onSubmit={handleAdd} className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              {/* Row 2: SKU, Quantity, Unit Price (NEW) */}
        ...
        {/* Suppliers Tab (Revised Create Form and List Display) */}
        {tab === 'suppliers' && (
          <GlassCard>
