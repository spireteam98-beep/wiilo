...
// --- Multi-Tenant Mock Data Simulation ---

// Mock data keyed by Business ID to simulate multi-tenancy
const mockDataStore = {
    // Example Business ID 1 (Assuming params.id = 'rest_001')
    'rest_001': {
        tables: [{ id: 'T01', status: 'Occupied' }, { id: 'T02', status: 'Available' }],
        staff: [{ id: 'W101', name: 'Waitress Amina' }, { id: 'C001', name: 'Cashier Fatima' }],
        financialSummary: { grossRevenue: 15450.00, netRevenue: 12500.00, cashBalance: 4500.00, pendingPayments: 750.00, apDue: 1200.00 },
        initialOrder: [
            { name: 'Cheeseburger', qty: 2, price: 12.00, discount: 0 },
            { name: 'Shisha (Double Apple)', qty: 1, price: 30.00, discount: 0 },
        ],
    },
    // Example Business ID 2 (Assuming params.id = 'rest_002')
    'rest_002': {
        tables: [{ id: 'S01', status: 'Occupied' }, { id: 'S02', status: 'Occupied' }, { id: 'S03', status: 'Available' }],
        staff: [{ id: 'W201', name: 'Waitress Jane' }, { id: 'K201', name: 'Chef Bob' }],
        financialSummary: { grossRevenue: 8000.00, netRevenue: 6500.00, cashBalance: 2100.00, pendingPayments: 200.00, apDue: 500.00 },
        initialOrder: [
            { name: 'Mint Mojito', qty: 3, price: 8.00, discount: 0 },
        ],
    }
    // Note: If the actual ID ('id') passed via params is not found, the default mock data will be used.
};

const getMultiTenantData = (businessId: string) => {
    return (mockDataStore as any)[businessId] || (mockDataStore as any)['rest_001']; 
};

// Mock Menu/Recipe remains global as it often is static inventory mapping
const mockMenuRecipes = {
    'Water': [{ item: 'Beef Patty', qty: 1, unit: 'ea' }, { item: 'Bun', qty: 1, unit: 'ea' }],
    'Shisha (Double Apple)': [{ item: 'Tobacco (DA)', qty: 50, unit: 'g' }, { item: 'Charcoal', qty: 3, unit: 'ea' }],
    'Orange Mint': [{ item: 'Mint Leaves', qty: 10, unit: 'g' }, { item: 'Lime Juice', qty: 50, unit: 'ml' }],
    'Milk Vanila': [{ item: 'Potatoes (Frozen)', qty: 150, unit: 'g' }],
};

const deductInventoryUsage = (orderItems: any[]) => {
    let totalDeductions = 0;
    // Implementation remains the same, but in a real app, this would use the Business ID to update the correct inventory ledger.
...
export default function BillingPage({ params }: { params: Promise<{ id: string }> | { id: string } }) {
  ...
  const { id } = (params as any) as { id: string }; // The Business ID passed via URL

  const [multiTenantData, setMultiTenantData] = useState(() => getMultiTenantData(id));
  const [tab, setTab] = useState<'pos' | 'accounting' | 'reports' | 'discounts'>('pos');
  const [inventoryDeducted, setInventoryDeducted] = useState(0); 
  
  // Use data loaded from the multi-tenant simulation
  const [currentTable, setCurrentTable] = useState(multiTenantData.tables[0]?.id || 'T01');
  const [currentOrder, setCurrentOrder] = useState(multiTenantData.initialOrder || []);
  ...
  // Ensure data reloads if the user or business ID changes (Multi-tenant check)
  useEffect(() => {
    // This is the CRITICAL STEP: When Business ID (id) changes, fetch the new data set.
    setMultiTenantData(getMultiTenantData(id));
    // Reset order state based on new business data
    setCurrentOrder(getMultiTenantData(id).initialOrder || []); 
    setCurrentTable(getMultiTenantData(id).tables[0]?.id || 'T01');
  }, [id]);
  ...
  const handleProcessPayment = () => {
      ...
      if (!staffId) return alert("ERROR: Staff ID must be selected to process payment/close order.");
      
      // In a real multi-tenant app, this API call would include the 'id' (business ID)
      alert(`STEP 7: Waiter (${staffId}) for Business [${id}] enters payment details. Total: $${finalTotal.toFixed(2)}. Pending Cashier Approval...`);

      const deductions = deductInventoryUsage(currentOrder);
      setInventoryDeducted(deductions);

      setTimeout(() => {
          alert(`STEP 8 & 9: Order Closed. Ledger Updated for Business [${id}]. ${deductions} raw item deductions logged.`);
          
          setCurrentOrder([]);
          setCurrentTable(multiTenantData.tables.find(t => t.status === 'Available')?.id || 'T01'); // Auto-select next available table
  ...
  const handlePrintKitchenReceipt = () => {
      if (!staffId) return alert("ERROR: Staff ID must be selected to print an order receipt.");
      if (currentOrder.length === 0) return alert("ERROR: Order is empty.");
      
      // Receipt printing is authenticated by Business ID and Staff ID
      alert(`STEP 3.1: Kitchen Receipt Printed for Business [${id}] by Staff ${staffId}.`);
  };
  ...
  return (
    <div className="p-6 text-white bg-black min-h-screen">
      <h1 className="text-3xl font-extrabold mb-6 text-[#F97316]">
        💰 Financial Management 
        <span className='text-xl text-white/50 ml-3'>[Business ID: {id}]</span>
      </h1>
      ...
      <div className="mt-6">
            ...
            <div className="col-span-1 md:col-span-2 space-y-6">
              ...
              <GlassCard>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
                    ...
                    {/* Staff Login / ID Selection (Step 1.1) */}
                    <GlassSelect 
                        value={staffId} 
                        onChange={e => setStaffId(e.target.value)}
                        className="col-span-1"
                    >
                        <option value="" className="bg-[#0F0F10]">-- Select Staff ID (Login) --</option>
                        {multiTenantData.staff.map((staff: any) => (
                            <option key={staff.id} value={staff.id} className="bg-[#0F0F10]">{staff.name} ({staff.id})</option>
                        ))}
                    </GlassSelect>

                    {/* Table Assignment (Step 1.1) */}
                    <GlassSelect value={currentTable} onChange={e => setCurrentTable(e.target.value)} className="col-span-1">
                        {multiTenantData.tables.map((t: any) => (
                            <option key={t.id} value={t.id} className={`bg-[#0F0F10] ${t.status === 'Occupied' ? 'text-red-400' : 'text-green-400'}`}>{t.id} - {t.status}</option>
                        ))}
                    </GlassSelect>
                    <GlassButton className="bg-white/20 hover:bg-white/30 border-none col-span-1">Split / Transfer Bill (2.1)</GlassButton>
                </div>
                {staffId && (
                    <p className='text-sm text-green-400 mt-2'>Logged in as Staff ID: **{staffId}**</p>
                )}
                {!staffId && (
                    <p className='text-sm text-yellow-400 mt-2'>Select Staff ID to start order and print tickets.</p>
                )}
                
                {/* Add Items (2.2) */}
                <h3 className="font-semibold mt-4 mb-2">Add Items to Order (2.2)</h3>
                <div className="grid grid-cols-1 sm:grid-cols-4 gap-3">
                    <GlassSelect value={newItemName} onChange={e => setNewItemName(e.target.value)} className="col-span-full sm:col-span-2">
                    ...
                    <GlassInput 
                        ...
                        className="col-span-1"
                    />
                    <GlassButton variant="accent" onClick={handleAddItem} disabled={!staffId} className="col-span-1">Add Item (2.2)</GlassButton>
                </div>
                ...
                <div className='max-h-60 overflow-y-auto space-y-2 p-2 rounded-lg border border-white/10'>
                    {currentOrder.map((item, index) => (
                        <div key={index} className="flex flex-wrap justify-between items-center p-2 bg-black/20 rounded-lg">
                            <span className='font-medium text-sm w-full sm:w-auto'>{item.name}</span>
                            
                            <div className='flex items-center gap-2 mt-2 sm:mt-0 w-full justify-end'>
                                
                                <span className='text-right font-medium mr-auto'>${(item.qty * item.price).toFixed(2)}</span>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                    <GlassButton 
                        ...
                        variant="accent" 
                        disabled={currentOrder.length === 0 || !staffId} 
                        className="w-full sm:w-auto flex-1"
                    >
                        Confirm & Print Kitchen Receipt (3.1)
                    </GlassButton>
                    <GlassButton disabled={currentOrder.length === 0 || !staffId} className="w-full sm:w-auto flex-1">
              ...
              <GlassCard>
                ...
                <div className="mt-4 flex flex-wrap gap-3">
                    <GlassButton variant="accent" disabled={currentOrder.length === 0} className="w-full sm:w-auto flex-1">Print Provisional Bill (6.2)</GlassButton>
                    <GlassButton className="w-full sm:w-auto flex-1">Add Special Instructions (2.3)</GlassButton>
                </div>
              </GlassCard>
            </div>
            
            {/* Right Column: Payment Management (7. Payment Workflow, 8. Cashier Approval) 
                Stacks last on mobile (col-span-1) */}
                ...
                <GlassCard className='h-full'>
                    ...
                    <div className="space-y-4">
                        ...
                        <div className="flex gap-2">
                            <GlassButton className="flex-1 w-full">Split Payment (7.2)</GlassButton>
                            <GlassButton variant="danger" className="flex-1 w-full">Refund / Void</GlassButton>
                        </div>
        ...
        {tab === 'accounting' && (
          <div className='grid grid-cols-1 md:grid-cols-2 gap-6'>
            ...
            <GlassCard>
              ...
              <div className="space-y-4">
                <div className='p-3 bg-white/10 rounded-lg'>
                    <h3 className="font-semibold text-white/70 mb-2">Outstanding Balances (Tenant: {id})</h3>
                    <div className="flex justify-between text-sm">
                        <span>Accounts Receivable (Customer Dues):</span> 
                        <span className='text-yellow-400'>${multiTenantData.financialSummary.pendingPayments.toFixed(2)}</span>
                    </div>
                    <div className="flex justify-between text-sm">
                        <span>Accounts Payable (Supplier Bills):</span> 
                        <span className='text-red-400'>${multiTenantData.financialSummary.apDue.toFixed(2)}</span>
                    </div>
                </div>
                
                <h3 className="font-semibold text-white/70 mb-2">Transaction Journal (Last 5)</h3>
                <div className='text-xs space-y-1 max-h-40 overflow-y-auto'>
                    <div className='p-2 bg-black/20 rounded-md'>Sale #1005 (Tenant: {id}) | Debit: Cash $154.00 | Credit: Sales $140.00, VAT $14.00</div>
            ...
            <GlassCard>
              ...
              <div className="space-y-4">
                <div className='p-3 bg-white/10 rounded-lg'>
                    <h3 className="font-semibold text-white/70 mb-2">Cash Register Summary (Tenant: {id})</h3>
                    <div className="flex justify-between text-lg">
                        <span>Current Cash on Hand:</span> 
                        <span className='text-green-400 font-bold'>${multiTenantData.financialSummary.cashBalance.toFixed(2)}</span>
                    </div>
                </div>
          ...
          <GlassCard>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 mb-6">
                <div className="bg-white/10 p-4 rounded-lg">
                    ...
                    <div className="text-2xl font-bold text-green-400">${multiTenantData.financialSummary.grossRevenue.toFixed(2)}</div>
                </div>
                 <div className="bg-white/10 p-4 rounded-lg">
                    <div className="text-sm text-white/70">Net Revenue (Today)</div>
                    <div className="text-2xl font-bold text-green-500">${multiTenantData.financialSummary.netRevenue.toFixed(2)}</div>
                </div>
                 <div className="bg-white/10 p-4 rounded-lg">
            ...
            <div className='grid grid-cols-1 sm:grid-cols-3 gap-3'>
                <GlassButton className="bg-white/20 hover:bg-white/30 w-full">P&L Statement (Monthly)</GlassButton>
                <GlassButton className="bg-white/20 hover:bg-white/30 w-full">Detailed Sales Report</GlassButton>
                <GlassButton className="bg-white/20 hover:bg-white/30 w-full">Cash Flow Projection</GlassButton>
                <GlassButton className="bg-white/20 hover:bg-white/30 w-full">Outstanding AR/AP</GlassButton>
                <GlassButton className="bg-white/20 hover:bg-white/30 w-full">Tax Summary</GlassButton>
                <GlassButton className="bg-white/20 hover:bg-white/30 w-full">Export All to CSV</GlassButton>
            </div>
        ...
        {tab === 'discounts' && (
            <GlassCard>
                ...
                <div className='grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6 p-4 bg-white/10 rounded-lg'>
                    ...
                    <div className="col-span-full flex gap-3 flex-wrap">
                        <GlassButton variant="accent" className="w-full sm:w-auto flex-1">Create Discount</GlassButton>
                        <GlassButton className="w-full sm:w-auto flex-1">View Active Offers</GlassButton>
                    </div>
                </div>
