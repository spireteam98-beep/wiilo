...
export function AudioCard({ audio, className }: AudioCardProps) {
  ...
  const handlePlay = (e: React.MouseEvent) => {
    e.stopPropagation();
    router.push(`/playlist?autoplay=true`);
  };
  ...
  const handleDownload = (e: React.MouseEvent) => {
    ...
    if (typeof window !== 'undefined') {
        ...
        if (!downloads.some((item: Audio) => item.id === audio.id)) {
    ...
    >
      <CardContent className="relative aspect-square p-0">
        ...
        <div className="absolute bottom-4 right-4 flex items-center justify-center opacity-0 transition-all duration-300 group-hover:opacity-100">
          <Button size="icon" className="h-12 w-12 bg-primary/80 backdrop-blur-sm hover:bg-primary" onClick={handlePlay}>
