  ...
  useEffect(() => {
    async function fetchRecommendations() {
      try {
        ...
        let recommendedAudios = allAudios.filter(audio => recommendedTitles.includes(audio.title.toLowerCase())).slice(0, 10);
        
        if (recommendedAudios.length < 3) {
            const currentRecIds = new Set(recommendedAudios.map(a => a.id));
            const filler = allAudios.filter(a => !currentRecIds.has(a.id)).slice(0, 10 - recommendedAudios.length);
        ...
        setRecommendations(allAudios.sort(() => 0.5 - Math.random()).slice(0, 10));
      } finally {
        setLoading(false);
      }
    }
  ...
  if (loading) {
    return (
        <section className="space-y-4">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 2xl:grid-cols-6 3xl:grid-cols-8 4xl:grid-cols-10 gap-4">
                {[...Array(10)].map((_, i) => (
      ...
      >
        <CarouselContent className="-ml-4">
          {recommendations.map((audio, index) => (
            <CarouselItem key={index} className="basis-1/2 pl-4 md:basis-1/3 lg:basis-1/4 xl:basis-1/5 2xl:basis-1/6 3xl:basis-1/8 4xl:basis-1/10">
